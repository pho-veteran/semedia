...
- [x] 12.A1 Verify eval can credit video/scene hits (runtime `scene:{filename}:{index}` vs `relevant_scene_ids`) — confirm which 0.0 slices are real; also check the `search_max_per_media=2` cap isn't dropping relevant scenes `[P0]`
- [x] 12.A2 Add configurable relevance score threshold (`search_min_score`) in `search_service.py` / `config.py` — fixes negative FP rate + precision; apply the floor after diversity so the candidate pool isn't starved `[P0]`
